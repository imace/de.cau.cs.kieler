...
import ssm.diagram.edit.commands.TransitionCreateCommand;
import ssm.diagram.edit.commands.TransitionReorientCommand;
 ...
 * @generated
 */
public class State3ItemSemanticEditPolicy extends SsmBaseItemSemanticEditPolicy {
	...
	protected void addDestroyChildNodesCommand(CompoundCommand cmd) {
		...
		for (Iterator it = view.getChildren().iterator(); it.hasNext();) {
			...
			switch (SsmVisualIDRegistry.getVisualID(node)) {
			case StateStateCompartment2EditPart.VISUAL_ID:
				for (Iterator cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (SsmVisualIDRegistry.getVisualID(cnode)) {
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (SsmElementTypes.Transition_4006 == req.getElementType()) {
