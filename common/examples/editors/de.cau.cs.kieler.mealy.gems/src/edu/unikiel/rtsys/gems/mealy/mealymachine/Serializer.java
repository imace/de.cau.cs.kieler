...
public class Serializer implements ModelSerializer{
	...
	public ModelInstance readModel(IFile dest, ClassLoader cl, IProgressMonitor monitor) throws Exception{
		...
		if(contents.size() > 0){
			...
			root = (edu.unikiel.rtsys.gems.mealy.mealymachine.Root)inst.getRoot();
			...
			root = new edu.unikiel.rtsys.gems.mealy.mealymachine.Root();
	...
	public void serializeToXMI(ModelObject obj, IFile dest) {
		...
		edu.unikiel.rtsys.gems.mealy.mealymachine.Root root = (edu.unikiel.rtsys.gems.mealy.mealymachine.Root)ModelRepository.getInstance().getInstanceRepository().getInstance(obj.getModelInstanceID()).getRoot();
