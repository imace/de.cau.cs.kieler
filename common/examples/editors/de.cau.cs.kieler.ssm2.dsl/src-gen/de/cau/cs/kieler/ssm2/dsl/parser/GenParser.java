...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private ssm2dlLexer lexer;
	private ssm2dlParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public ssm2dlLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new ssm2dlLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public ssm2dlParser getParser() {
		if (parser == null) {
			parser = new ssm2dlParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "de::cau::cs::kieler::ssm2::dsl::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"de::cau::cs::kieler::ssm2::dsl::GenChecks","de::cau::cs::kieler::ssm2::dsl::Checks"};
