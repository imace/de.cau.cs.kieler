...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	public KITLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new KITLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public KITParser getParser() {
		if (parser == null) {
			parser = new KITParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "de::cau::cs::kieler::ssm2::kit::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"de::cau::cs::kieler::ssm2::kit::GenChecks","de::cau::cs::kieler::ssm2::kit::Checks"};
