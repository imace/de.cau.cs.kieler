...
public class LayoutGraphCanvas extends Canvas implements PaintListener {
    ...
    public class PaintRectangle {
        ...
        PaintRectangle(KShapeLayout shapeLayout, KPoint offset) {
            this.x = Math.round(shapeLayout.getLocation().getX()
                    + offset.getX());
            this.y = Math.round(shapeLayout.getLocation().getY()
                    + offset.getY());
            this.width = Math.max(Math.round(shapeLayout.getSize().getWidth()),
                    3);
            this.height = Math.max(Math
                    .round(shapeLayout.getSize().getHeight()), 3);
        }
    ...
    /** the layout graph */
    private List<KLayoutGraph> layoutGraph = null;
     ...
     * @param layoutGraph2
     */
    public void addLayoutGraph(KLayoutGraph layoutGraph) {
        // set new size values for the canvas
        if (layoutGraph != null) {
            KDimension graphDim = layoutGraph.getLayout().getSize();
            setSize(new Point((int) graphDim.getWidth() + 1, (int) graphDim
                    .getHeight() + 1));
        }

        if (this.layoutGraph == null)
            this.layoutGraph = new ArrayList<KLayoutGraph>();
    ...
    public void paintControl(PaintEvent event, int color) {
        if (layoutGraph != null && layoutGraph.size() > 0) {
            ...
            for (KLayoutGraph graph : layoutGraph) {
                if(graph!=null){
    ...
    private void paintLayoutNode(KLayoutNode layoutNode, GC graphics,
        ...
        for (KLayoutPort port : layoutNode.getPorts()) {
            ...
            if (paintLabels && port.getLabel() != null) {
                KPoint portOffset = KimlLayoutGraphFactory.eINSTANCE
                        .createKPoint();
                portOffset.setX(rect.x);
                portOffset.setY(rect.y);
                rect = boundsMap.get(port.getLabel());
                if (rect == null) {
                    rect = new PaintRectangle(port.getLabel().getLabelLayout(),
                            portOffset);
        ...
        for (KLayoutNode child : layoutNode.getChildNodes()) {
            ...
            KPoint childOffset = KimlLayoutGraphFactory.eINSTANCE
                    .createKPoint();
            ...
            if (paintLabels && child.getLabel() != null) {
                ...
                if (rect == null && child.getLabel().getLabelLayout() != null) {
                    rect = new PaintRectangle(
                            child.getLabel().getLabelLayout(), childOffset);
                    boundsMap.put(child.getLabel(), rect);
                } else if (rect == null
                        && child.getLabel().getLabelLayout() == null) {
                    rect = new PaintRectangle(child.getLayout(), subOffset);
                }
                if (!rect.painted && rect.intersects(area)) {
     ...
     *            offset to be added to relative coordinates
     */
    private void paintEdge(KLayoutEdge edge, GC graphics, PaintRectangle area,
            KPoint offset) {
        ...
        if (paintLabels) {
            for (KEdgeLabel edgeLabel : edge.getLabel()) {
                ...
                if (rect == null) {
                    rect = new PaintRectangle(edgeLabel.getLabelLayout(),
                            offset);
