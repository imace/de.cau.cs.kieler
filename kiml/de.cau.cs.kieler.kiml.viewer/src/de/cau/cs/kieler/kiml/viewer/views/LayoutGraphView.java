...
public class LayoutGraphView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		// create canvas for XMI resource
		{
		    ...
		    ScrolledComposite xmiScroller = new ScrolledComposite(tabFolder,
		            SWT.H_SCROLL | SWT.V_SCROLL);
		    ...
		    getSite().getPage().addPartListener(new IPartListener() {
                public void partActivated(IWorkbenchPart part) {
                    if (part instanceof IEditorPart) {
                        IEditorInput editorInput = ((IEditorPart)part)
                                .getEditorInput();
                        if (editorInput instanceof IFileEditorInput) {
                            String path = ((IFileEditorInput)editorInput)
		...
		tabFolder.addSelectionListener(new SelectionListener() {
            public void widgetDefaultSelected(SelectionEvent e) {
                preferenceStore.setValue(PREF_ACTIVE_TAB,
                        tabFolder.indexOf((TabItem)e.item));
            }
