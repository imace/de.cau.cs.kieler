...
public class ApplyLayoutEditPolicy extends AbstractEditPolicy {
    ...
    private void addShapeLayouts(ApplyLayoutCommand command,
        ...
        if (nodeMap != null) {
            for (KNode knode : nodeMap.keySet()) {
                ...
                addShapeLayout(command, editPart, nodeLayout);
            }
        }
        ...
        if (portMap != null) {
            for (KPort kport : portMap.keySet()) {
                ...
                addShapeLayout(command, editPart, portLayout);
            }
        }
        
        // process layout data of edge labels
        Map<KLabel, LabelEditPart> edgeLabelMap = request.getEdgeLabelMap();
        if (edgeLabelMap != null) {
            for (KLabel klabel : edgeLabelMap.keySet()) {
                KShapeLayout labelLayout = KimlLayoutUtil.getShapeLayout(klabel);
                LabelEditPart labelEditPart = edgeLabelMap.get(klabel);
    ...
    private void addEdgeLayouts(ApplyLayoutCommand command,
            ApplyLayoutRequest request) {
        Map<KEdge, ConnectionEditPart> edgeMap = request.getEdgeMap();
        if (edgeMap != null) {
            for (KEdge kedge : edgeMap.keySet()) {
                KEdgeLayout edgeLayout = KimlLayoutUtil.getEdgeLayout(kedge);
                ConnectionEditPart connectionEditPart = edgeMap.get(kedge);
                PointList bendPoints = getBendPoints(edgeLayout);
