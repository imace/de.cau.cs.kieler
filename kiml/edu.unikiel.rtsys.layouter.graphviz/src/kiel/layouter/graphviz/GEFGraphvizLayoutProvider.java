 ...
 * 
 */
public class GEFGraphvizLayoutProvider extends DefaultProvider{
...
//		CompoundCommand cc = new CompoundCommand(); // dummy command to return nothing for debugging
					...
					for (Object child : con.getChildren()) {
					    ...
					    if (child instanceof GraphicalEditPart) {
							...
							if(edge.getEdgeLabel() == null){ 
								...
								GraphicalEditPart labelEP = (GraphicalEditPart) child;
	...
	private ArrayList buildNodes(List selectedEditParts,
		...
		while (li.hasNext()) {
			...
			System.out.println("G E P: " + gep);
			System.out.println("Model: " + gep.getModel());
			// make sure the child is a shape (instead of a connection) and not a border item ("port") 
			if (gep instanceof NodeEditPart ){//&& !(gep instanceof IBorderItemEditPart)) {
				NodeEditPart nodeEP = (NodeEditPart) gep;
				...
				node.setData(nodeEP); 
				editPart2Node.put(nodeEP, node);
				// store current dimension of EP in graph model
				Size size = factory.createSize();
				size.setWidth(nodeEP.getFigure().getSize().width);
				size.setHeight(nodeEP.getFigure().getSize().height);
				node.setSize(size);
...
*/			
				...
				System.out.println("BeforeLabelLabel");
						
				Label label = edge.getEdgeLabel();
				if (label != null && label.getData() instanceof GraphicalEditPart) {
					System.out.println("LabelLabel");
					GraphicalEditPart labelEP = (GraphicalEditPart) label.getData();
	...
	private void updateDiagramNodes(CompositeNode parentNode, CompoundCommand cc) {
		for (Node node : parentNode.getNodes()) { // get all child nodes to move
			System.out.println("updateDiagramNodes");
			if (node.getData() instanceof GraphicalEditPart) { // check if node is a Graphical EditPart
				System.out.println("GraphicalEditPart");
				GraphicalEditPart gep = (GraphicalEditPart) node.getData();
