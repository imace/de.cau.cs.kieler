...
/**
 * Controls existing extensions for the KIML Diagram Layouter extension
 * point. 
 * 
 * @author msp
 */
public final class DiagramLayouters {
	...
	private void loadAvailableLayouters() {
		...
		for (IConfigurationElement element : extensions) {
			try {
				...
				String editorId = element.getAttribute(KimlAbstractLayouter.ATTRIBUTE_EDITOR_ID);
	 ...
	 * @param editorId ID string for the editor
	 * @return an instance of the diagram layouter, or null if no layouter is
	 *   available for the given editor
	 */
	public KimlAbstractLayouter getDiagramLayouter(String editorId) {
		return diagramLayouterMap.get(editorId);
	}
