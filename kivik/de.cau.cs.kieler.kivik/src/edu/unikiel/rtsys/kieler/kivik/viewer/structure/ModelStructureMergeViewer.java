...
public class ModelStructureMergeViewer extends TreeViewer {
	 ...
	 * caused by a selection event in the structure viewer.
	 */
	 ...
	 */
	public ModelStructureMergeViewer(Composite parent,
	...
	public String getTitle() {
		return EMFCompareUIMessages
				.getString("ModelStructureMergeViewer.viewerTitle"); //$NON-NLS-1$
	}

	/**
	 * This will initialize the "save as emfdiff" action and put its icon in the
	...
	/**
	 * Updates the Structure viewer's tool items. This will modify the actions
	 * of the "export diff as..." menu.
	 */
	protected void updateToolItems() {
		// exportMenu.enableSave(!ModelComparator.getComparator(configuration).
		// isLeftRemote()
	...
	/**
	 * initializer of this structure merge viewer. It sets the
	 * {@link LabelProvider label} and content provider for the tree and creates
	 * the different needed listeners.
	 * 
	 * @param compareConfiguration
	...
	private void initialize(CompareConfiguration compareConfiguration) {
		...
		configuration
				.addPropertyChangeListener(new ConfigurationPropertyListener());
	}

	/**
	 * Listens to property change events on the compare configuration. Typically
	 * responds to selection change events in the content viewer and preferences
	 * change.
	 * 
	 * @author <a href="mailto:laurent.goubet@obeo.fr">Laurent Goubet</a>
	 */
	private class ConfigurationPropertyListener implements
		...
		public void propertyChange(PropertyChangeEvent event) {
			if (event.getProperty().equals(
					EMFCompareConstants.PROPERTY_CONTENT_SELECTION)) {
				if (ignoreContentSelection)
					ignoreContentSelection = false;
				else {
					TreeItem item = (TreeItem) find(event.getNewValue());
					/*
					 * if we could not find the item, we need to expand the
					 * whole tree to try and get it. This is due to the lazy
					 * loading of the tree content.
					 */
					if (item == null) {
					...
					EMFCompareConstants.PROPERTY_CONTENT_INPUT_CHANGED)) {
	...
	private class ModelStructureLabelProvider extends LabelProvider {
		/**
		 * We use this generic label provider, but we want to customize some
		 * aspects that's why we choose to aggregate it.
		 */
		...
		public ModelStructureLabelProvider() {
			adapterProvider = new AdapterFactoryLabelProvider(AdapterUtils
					.getAdapterFactory());
		 ...
		 * {@link IFile}, delegates to the {@link AdapterFactoryLabelProvider}
		 * to get the {@link Image}.
		 * 
		 * @param object
