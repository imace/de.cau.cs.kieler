...
public final class ModelContentMergePropertyTab extends TableViewer implements
	...
	private final Map<String, ModelContentMergeTabObject> dataToItem = new EMFCompareMap<String, ModelContentMergeTabObject>();

	/**
	 * Keeps a reference to all the differences detected by the comparison. This
	 * list will be cleared as soon as {@link #dataToItem} will be populated.
	 */
	...
	public ModelContentMergePropertyTab(Composite parentComposite, int side,
		...
		setLabelProvider(new PropertyLabelProvider(AdapterUtils
				.getAdapterFactory()));
		...
		nameColumn.setText(EMFCompareUIMessages
				.getString("ModelContentMergePropertyPart.column1.name")); //$NON-NLS-1$
		nameColumn.setWidth(Dialog.convertWidthInCharsToPixels(metrics,
				nameColumn.getText().length() * 3));
		final TableColumn weightsColumn = new TableColumn(getTable(), SWT.RIGHT);
		weightsColumn.setText(EMFCompareUIMessages
				.getString("ModelContentMergePropertyPart.column2.name")); //$NON-NLS-1$
	...
	/**
	 * Returns the widget representing the given {@link DiffElement} in the
	 * table.
	 * 
	 * @param diff
	...
	public TableItem find(DiffElement diff) {
		final EObject inputEObject = ((PropertyContentProvider) getContentProvider())
				.getInputEObject();
		TableItem item = null;
	...
	public ModelContentMergeTabObject getUIElement(EObject data) {
		...
		final ModelContentMergeTabObject item = dataToItem.get(key);
		if (item != null) {
			if (getSelectedElements().contains(item.getActualObject()))
				item.setCurveSize(2);
			else
				item.setCurveSize(1);
			item.setCurveY(((TableItem) item.getActualObject()).getBounds().y
					+ ((TableItem) item.getActualObject()).getBounds().height
					/ 2);
		}
		return item;
	}
	 ...
	 * <p>
	 * For the property tab, we won't try and find the "visible" elements and
	 * this will return the whole table's content.
	 * </p>
	 * 
	 * @return List of this tab's elements.
	 */
	public List<ModelContentMergeTabObject> getVisibleElements() {
		if (dataToItem.size() > 0
				&& dataToItem.values().iterator().next().getActualObject() instanceof Item
				&& ((Item) dataToItem.values().iterator().next()
						.getActualObject()).isDisposed())
			mapTableItems();
		...
		for (String data : dataToItem.keySet()) {
			...
			if (data.startsWith(EcoreUtil.getURI(element).fragment())) {
				final ModelContentMergeTabObject next = dataToItem.get(data);
				final TableItem nextTableItem = (TableItem) next
						.getActualObject();
				if (nextTableItem.getBounds().y >= getTable().getClientArea().y
						&& nextTableItem.getBounds().y <= getTable()
								.getClientArea().y
					...
					if (getSelectedElements().contains(nextTableItem))
						...
						next.setCurveSize(1);
					next.setCurveY(nextTableItem.getBounds().y
							+ nextTableItem.getBounds().height / 2);
	...
	protected void associate(Object element, Item item) {
		if (element instanceof Object[]) {
			...
			if (element instanceof List) {
				...
				// Always map the element, even if data ==
				// ((List)element).get(0),
				// since unmapAllElements() can leave the map inconsistent
				// See bug 2741 for details.
	...
	private void mapDifferences() {
		...
		while (diffIterator.hasNext()) {
			...
			if (diff instanceof ReferenceChange
					|| diff instanceof AttributeChange) {
	...
	/**
	 * This will map all the TableItems in this TreeViewer that need be taken
	...
	private void mapTableItems() {
		...
		for (int i = 0; i < differences.size(); i++) {
			...
			if (item != null) {
				...
				final ModelContentMergeTabObject wrappedItem = new ModelContentMergeTabObject(
						item, color);
				String key = ""; //$NON-NLS-1$
				if (partSide == EMFCompareConstants.LEFT)
					key = EcoreUtil.getURI(
							EMFCompareEObjectUtils.getLeftElement(diff))
							.fragment();
				else if (partSide == EMFCompareConstants.RIGHT)
					key = EcoreUtil.getURI(
							EMFCompareEObjectUtils.getRightElement(diff))
							.fragment();
				final String[] fragments = EcoreUtil.getURI(data).fragment()
						.split("/"); //$NON-NLS-1$
				key += '/' + fragments[fragments.length - 1];
	...
	private class PropertyLabelProvider extends AdapterFactoryLabelProvider {
		 ...
		 *            Adapter factory providing this {@link LabelProvider}'s
		 *            text and images.
		 */
		...
		public Image getColumnImage(Object object, int columnIndex) {
			...
			if (object instanceof List) {
				image = super.getColumnImage(((List<?>) object)
		...
		public String getColumnText(Object object, int columnIndex) {
			...
			if (object instanceof List) {
				text = super.getColumnText(((List<?>) object).get(columnIndex),
						columnIndex);
			}
			return text;
		}
