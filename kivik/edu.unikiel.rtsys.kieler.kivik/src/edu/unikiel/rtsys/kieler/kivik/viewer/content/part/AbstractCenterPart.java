...
/**
 * We want to avoid flickering as much as possible for our draw operations on
 * the center part, yet we can't use double buffering to draw on it. We will
 * then draw on a {@link Canvas} moved above that center part.
 * 
...
public abstract class AbstractCenterPart extends Canvas {
	 ...
	 */
	protected void drawLine(GC gc, ModelContentMergeTabObject leftItem,
			ModelContentMergeTabObject rightItem) {
		...
		if (System.getProperty("os.name").contains("Windows")) { //$NON-NLS-1$ //$NON-NLS-2$
			...
			leftY = leftItem.getCurveY() + leftItem.getHeaderHeight()
					+ treeTabBorder;
			rightY = rightItem.getCurveY() + rightItem.getHeaderHeight()
					+ treeTabBorder;
		}
	...
	protected List<DiffElement> retainVisibleDiffs(List<DiffElement> diffList,
			List<ModelContentMergeTabObject> leftVisible,
			List<ModelContentMergeTabObject> rightVisible) {
		final List<DiffElement> visibleDiffs = new ArrayList<DiffElement>(
				diffList.size());
				
		if (leftVisible == null || rightVisible == null)
			return visibleDiffs;

		for (ModelContentMergeTabObject left : leftVisible) {
			for (DiffElement nextDiff : diffList) {
				if (EMFCompareEObjectUtils.getLeftElement(nextDiff) == left
						.getActualObject()) {
		...
		for (ModelContentMergeTabObject right : rightVisible) {
			for (DiffElement nextDiff : diffList) {
