...
public GemsWizardPage(IWorkbench aWorkbench, IStructuredSelection selection) {
	super("MealyMachine DSML", selection);  
	this.setTitle("MealyMachine DSML");
	this.setDescription("Create a MealyMachine DSML instance");
...
public void createControl(Composite parent) {
	...
	this.setFileName("MealyMachine" + count + ".gemsmealy"); 
...
protected InputStream getInitialContents() {
	...
	try{
			...
			String intial = "<?xml version=\"1.0\" encoding=\"ASCII\"?>\n<mealymachine:Root xmlns:mealymachine=\"http://www.informatik.uni-kiel.de/rtsys/gems/mealy\"></mealymachine:Root>";
...
public boolean finish() {
	...
	InitialContentProvider[] initcontents = PluginUtilities.getInitialContentProviders(MealyMachineProvider.MODEL_ID);
	for(InitialContentProvider init : initcontents)
		init.provideContent(MealyMachineProvider.getInstance(),newFile, useroptions);
