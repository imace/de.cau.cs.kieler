 ...
 * @generated
 */
public class CFBTypeItemSemanticEditPolicy extends
		CakefeedBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public CFBTypeItemSemanticEditPolicy() {
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (CakefeedElementTypes.IFInputEvent_3021 == req.getElementType()) {
			return getGEFWrapper(new IFInputEventCreateCommand(req));
		}
		if (CakefeedElementTypes.IFOutputEvent_3022 == req.getElementType()) {
			return getGEFWrapper(new IFOutputEventCreateCommand(req));
		}
		if (CakefeedElementTypes.IFInputVar_3023 == req.getElementType()) {
			return getGEFWrapper(new IFInputVarCreateCommand(req));
		}
		if (CakefeedElementTypes.IFOutputVar_3024 == req.getElementType()) {
		...
		for (Iterator nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case IFInputVarEditPart.VISUAL_ID:
				for (Iterator it = node.getSourceEdges().iterator(); it
						.hasNext();) {
					Edge outgoingLink = (Edge) it.next();
					if (CakefeedVisualIDRegistry.getVisualID(outgoingLink) == IFIFDataConnectionEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								outgoingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
					...
					if (CakefeedVisualIDRegistry.getVisualID(outgoingLink) == IFFBDataConnectionEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								outgoingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
			...
			case IFOutputVarEditPart.VISUAL_ID:
				for (Iterator it = node.getTargetEdges().iterator(); it
						.hasNext();) {
					Edge incomingLink = (Edge) it.next();
					if (CakefeedVisualIDRegistry.getVisualID(incomingLink) == OutputWithEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
					...
					if (CakefeedVisualIDRegistry.getVisualID(incomingLink) == IFIFDataConnectionEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
					...
					if (CakefeedVisualIDRegistry.getVisualID(incomingLink) == FBIFDataConnectionEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
			...
			case CFBTypeCFBTypeCompartmentEditPart.VISUAL_ID:
				for (Iterator cit = node.getChildren().iterator(); cit
					...
					switch (CakefeedVisualIDRegistry.getVisualID(cnode)) {
