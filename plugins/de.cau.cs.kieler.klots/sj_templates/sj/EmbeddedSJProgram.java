...
import static sj.SJLogger.LogMsgTyp.*;
import sj.SJLogger.LogMsgTyp;
...
public abstract class EmbeddedSJProgram<StateLabel extends Enum<?>> {
    ...
    public Signal[] doTick(final Signal... inputs) {
        ...
        // ############################################################### //
        // FIXME: List.toArray() Method in leJOS is BUGGY!!!
        // ret = signals.toArray(ret);
        int count = 0;
		for(Signal elem : signals) {
			ret[count] = elem;
			count++;
		}
        // ############################################################### //
