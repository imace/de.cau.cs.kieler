 ...
 */
/*package*/
class RasterFileExportOperation implements IRunnableWithProgress {
	private IPath path;
	private IProgressMonitor monitor;
	private RasterFileExporter exporter = new RasterFileExporter(RasterFormat.JPEG);
	 ...
	 */
	public RasterFileExportOperation(
		List resources,
		String destinationPath,
		LinkedList optionsList,
		String suffix,
		...
		exporter.setOptions(optionsList);
	...
	public RasterFileExportOperation(
		...
		String suffix,
		IOverwriteQuery overwriteImplementor) {
		this(res, destinationPath, optionsList, suffix, overwriteImplementor);
		resourcesToExport = resources;
	}
	/**
	...
	protected void exportFile(IFile file, IPath location)
		...
		if (targetFile.exists()) {
			if (!targetFile.canWrite()) {
				errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0, Messages.format("RasterFileExportOperation.cannotOverwrite", //$NON-NLS-1$
		...
		try {
			...
			e.printStackTrace();
			errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0, Messages.format("RasterFileExportOperation.errorExporting", //$NON-NLS-1$
	...
	public IStatus getStatus() {
		...
		return new MultiStatus(PlatformUI.PLUGIN_ID, IStatus.OK, errors, Messages.getString("RasterFileExportOperation.problemsExporting"), //$NON-NLS-1$
		null);
	}
	/**
	...
	public void run(IProgressMonitor progressMonitor) throws InterruptedException {
		...
		try {
			...
			monitor.beginTask(Messages.getString("RasterFileExportOperation.exportingTitle"), totalWork); //$NON-NLS-1$
			if (resourcesToExport == null) {
				exportAllResources();
			} else {
