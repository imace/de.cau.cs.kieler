...
#include <stdlib.h>
#include <string.h>
#include "U:\shared\modelgui\trunk\scade6_example\Simulation\simulation__Simulation_type.h"
...
/* Print/Set function implementation for type ControlDataType__elevator */



struct SimTypeVTable* pSimControlDataType__elevatorVTable;
void print_ControlDataType__elevator_value(void* p){
	if (pSimControlDataType__elevatorVTable) {
		print_token (*(char**)(void*)pSimControlDataType__elevatorVTable->m_pfnToType(SptString, p));
	}
	else {
...
int set_ControlDataType__elevator(value* _v, void* p){
	int SAO_ok = 0;
	if (pSimControlDataType__elevatorVTable && pSimControlDataType__elevatorVTable->m_pfnGetConvInfo(SptNone, SptString))
		SAO_ok = pSimControlDataType__elevatorVTable->m_pfnFromType(SptString, &_v->u.a_token, p);
	else {
...
void print_ControlDataType__elevator_type(void){
	print_string("ControlDataType__elevator");
}
...
/* Print/Set function implementation for type DisplayDataType__elevator */



struct SimTypeVTable* pSimDisplayDataType__elevatorVTable;
void print_DisplayDataType__elevator_value(void* p){
	if (pSimDisplayDataType__elevatorVTable) {
		print_token (*(char**)(void*)pSimDisplayDataType__elevatorVTable->m_pfnToType(SptString, p));
	}
	else {
...
int set_DisplayDataType__elevator(value* _v, void* p){
	int SAO_ok = 0;
	if (pSimDisplayDataType__elevatorVTable && pSimDisplayDataType__elevatorVTable->m_pfnGetConvInfo(SptNone, SptString))
		SAO_ok = pSimDisplayDataType__elevatorVTable->m_pfnFromType(SptString, &_v->u.a_token, p);
	else {
...
void print_DisplayDataType__elevator_type(void){
	print_string("DisplayDataType__elevator");
}
